...
def _resolve_attn_implementation_for_hf_loading(
    attn_implementation: str, model_dtype: Optional[torch.dtype] = None
) -> str:
    """Fallback to sdpa when flash_attention_2 is unavailable or ABI-incompatible."""
    if attn_implementation != "flash_attention_2":
        return attn_implementation

    if model_dtype not in (None, torch.float16, torch.bfloat16):
        logger.warning(
    ...
    # `is_flash_attn_2_available()` can return True even if the CUDA extension
    # is ABI-incompatible with current torch/cuda. Probe import explicitly.
    try:
...
def load_valuehead_model(local_path, torch_dtype, model_config, trust_remote_code):
    ...
    attn_implementation = _resolve_attn_implementation_for_hf_loading(
        attn_implementation=attn_implementation, model_dtype=torch_dtype
    )
