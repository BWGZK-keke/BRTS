    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "swanlab" in default_backend:
            ...
            try:
                ...
                print(
                    f"[WARN] SwanLab init failed ({type(exc).__name__}: {exc}). "
                    "Continue without SwanLab logging."
                )
                if "console" not in self.logger:
        ...
        for default_backend, logger_instance in list(self.logger.items()):
            if backend is None or default_backend in backend:
                try:
                    ...
                    print(
                        f"[WARN] Logging to backend '{default_backend}' failed at step {step} "
                        f"({type(exc).__name__}: {exc}). Disabling this backend for the rest of the run."
                    )
                    failed_backends.append(default_backend)

        for failed_backend in failed_backends:
            logger_instance = self.logger.pop(failed_backend, None)
            if logger_instance is None:
                continue
            try:
                if failed_backend == "wandb":
                    logger_instance.finish(exit_code=1)
                elif failed_backend == "swanlab":
                    logger_instance.finish()
                elif failed_backend == "vemlp_wandb":
                    logger_instance.finish(exit_code=1)
                elif failed_backend in {"tensorboard", "clearml", "trackio", "file"}:
                    logger_instance.finish()
            except Exception:
                pass
