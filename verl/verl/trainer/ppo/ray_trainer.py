                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.use_rm and "rm_scores" not in batch.batch.keys():
                            ...
                            # --------------------------------------------------
                            # Teacher Rollout + tiered selection (optional)
                            # The selected teacher rollout can be used as teacher
                            # forward context for reward/distillation computation
                            # when apply_selected_for_teacher_forward=True:
                            #   tier 1 — unhinted rollouts, correct + most-similar
                            #   tier 2 — hinted retry for samples with no correct tier-1
                            #   tier 3 — most-similar tier-1 rollout when neither tier
                            #            yields a correct answer
                            # --------------------------------------------------
                            teacher_rollout_cfg = self.config.actor_rollout_ref.rollout.get(
                                "teacher_rollout", {}
                            )
                            teacher_pad_id = (
                            ...
                            if teacher_pad_id is None:
                                teacher_pad_id = 0
                            selected_teacher_responses = None
                            apply_selected_for_teacher_forward = False
                            multi_forward_select_by_overlap = False
                            if teacher_rollout_cfg.get("enable", False):
                                from verl.trainer.ppo.teacher_selection import (
                                    finalize_selection,
                                    select_teacher_tier1,
                                )

                                n_rollouts_tier1 = teacher_rollout_cfg.get("n_rollouts", 4)
                                # Backward-compatible alias support:
                                #   tier1_only (preferred) / first_tier_only (legacy)
                                tier1_only = bool(
                                    teacher_rollout_cfg.get(
                                        "tier1_only",
                                        teacher_rollout_cfg.get("first_tier_only", False),
                                    )
                                )
                                n_rollouts_tier2_cfg = teacher_rollout_cfg.get("n_rollouts_hint", 2)
                                n_rollouts_tier2 = 0 if tier1_only else n_rollouts_tier2_cfg
                                # Keep teacher forward context unchanged by default.
                                # Set this flag true to restore/use selected-rollout forwarding.
                                apply_selected_for_teacher_forward = bool(
                                    teacher_rollout_cfg.get(
                                        "apply_selected_for_teacher_forward",
                                        teacher_rollout_cfg.get("use_selected_rollout_for_teacher_forward", False),
                                    )
                                )
                                # Run multiple teacher forward trials (no extra generation),
                                # then pick the better forward result by overlap.
                                multi_forward_select_by_overlap = bool(
                                    teacher_rollout_cfg.get(
                                        "multi_forward_select_by_overlap",
                                        teacher_rollout_cfg.get("run_multi_forward", False),
                                    )
                                )
                                batch.meta_info["teacher_n_rollouts"] = n_rollouts_tier1
                                batch.meta_info["teacher_n_rollouts_hint"] = n_rollouts_tier2
                                batch.meta_info["teacher_tier1_only"] = tier1_only
                                batch.meta_info["teacher_max_new_tokens"] = teacher_rollout_cfg.get(
                                    "max_new_tokens", 2048
                                )
                                batch.meta_info["teacher_gen_temperature"] = teacher_rollout_cfg.get(
                                    "temperature", 0.7
                                )
                                batch.meta_info["teacher_top_p"] = teacher_rollout_cfg.get(
                                    "top_p", 0.95
                                )
                                batch.meta_info["teacher_prompt_disturb_enable"] = teacher_rollout_cfg.get(
                                    "prompt_disturb_enable", False
                                )
                                batch.meta_info["teacher_prompt_disturb_rollout_index"] = teacher_rollout_cfg.get(
                                    "prompt_disturb_rollout_index", 2
                                )
                                batch.meta_info["teacher_prompt_disturb_text"] = teacher_rollout_cfg.get(
                                    "prompt_disturb_text", ""
                                )

                                with marked_timer("teacher_rollout_tier1", timing_raw, color="teal"):
                                    teacher_rollouts_t1 = self.rm_wg.generate_teacher_rollouts(batch)

                                with marked_timer("teacher_selection_tier1", timing_raw, color="lime"):
                                    tier1_state, tier1_metrics = select_teacher_tier1(
                                        batch=batch,
                                        teacher_rollouts=teacher_rollouts_t1,
                                        n_rollouts=n_rollouts_tier1,
                                        tokenizer=self.tokenizer,
                                    )
                                    metrics.update(tier1_metrics)
                                    metrics["teacher_selection/tier1_only_mode"] = 1.0 if tier1_only else 0.0

                                teacher_rollouts_t2 = None
                                n_retry = int(tier1_state["retry_indices"].numel())
                                if n_retry > 0 and n_rollouts_tier2 > 0:
                                    with marked_timer("teacher_rollout_tier2", timing_raw, color="teal"):
                                        # Run hinted tier-2 rollouts on the full batch.
                                        teacher_rollouts_t2 = self.rm_wg.generate_teacher_rollouts_with_hint(batch)

                                with marked_timer("teacher_selection_final", timing_raw, color="lime"):
                                    final_selected_ids, tier_tags, tier_metrics = finalize_selection(
                                        batch=batch,
                                        tier1_state=tier1_state,
                                        tier2_rollouts=teacher_rollouts_t2,
                                        n_rollouts_tier2=n_rollouts_tier2,
                                        tokenizer=self.tokenizer,
                                    )
                                    metrics.update(tier_metrics)
                                    print(
                                        f"[TeacherRollout] step {self.global_steps}: "
                                        f"tier1 {tier_metrics['teacher_selection/frac_tier1']:.2f} / "
                                        f"tier2 {tier_metrics['teacher_selection/frac_tier2']:.2f} / "
                                        f"tier3 {tier_metrics['teacher_selection/frac_tier3']:.2f} "
                                        f"(retries={n_retry}/{tier_tags.shape[0]}, "
                                        f"tier1_only={int(tier1_only)})"
                                    )

                                # Align selected teacher rollouts to response length so they can
                                # optionally replace student responses in teacher forward.
                                response_len = batch.batch["responses"].shape[1]
                                if final_selected_ids.shape[1] < response_len:
                                    pad = torch.full(
                                        (final_selected_ids.shape[0], response_len - final_selected_ids.shape[1]),
                                        teacher_pad_id,
                                        dtype=final_selected_ids.dtype,
                                    )
                                    final_selected_ids = torch.cat([final_selected_ids, pad], dim=1)
                                elif final_selected_ids.shape[1] > response_len:
                                    final_selected_ids = final_selected_ids[:, :response_len]
                                selected_teacher_responses = final_selected_ids.to(
                                    device=batch.batch["responses"].device,
                                    dtype=batch.batch["responses"].dtype,
                                )
                                metrics["teacher_selection/selected_non_pad_frac"] = float(
                                    (selected_teacher_responses != teacher_pad_id).float().mean().item()
                                )
                                metrics["teacher_selection/apply_selected_for_teacher_forward"] = (
                                    1.0 if apply_selected_for_teacher_forward else 0.0
                                )
                                metrics["teacher_selection/multi_forward_select_by_overlap"] = (
                                    1.0 if multi_forward_select_by_overlap else 0.0
                                )
                            ...
                            with marked_timer("compute_rm_score", timing_raw, color="magenta"):
                                def _build_selected_forward_batch(
                                    base_batch: DataProto, include_all_batch_keys: bool = True
                                ) -> DataProto:
                                    # Keep PPO tensors on student responses, but prepare a copy
                                    # where teacher forward runs on selected teacher responses.
                                    non_tensor_keys = list(base_batch.non_tensor_batch.keys()) if base_batch.non_tensor_batch else None
                                    if include_all_batch_keys:
                                        batch_keys = list(base_batch.batch.keys())
                                    else:
                                        batch_keys = ["input_ids", "attention_mask", "position_ids", "responses"]
                                        if "response_mask" in base_batch.batch.keys():
                                            batch_keys.append("response_mask")
                                    out = base_batch.select(
                                        batch_keys=batch_keys,
                                        non_tensor_batch_keys=non_tensor_keys,
                                        meta_info_keys=list(base_batch.meta_info.keys()),
                                    )
                                    response_len = selected_teacher_responses.shape[1]
                                    out.batch["responses"] = selected_teacher_responses

                                    teacher_input_ids = base_batch.batch["input_ids"].clone()
                                    teacher_input_ids[:, -response_len:] = selected_teacher_responses
                                    out.batch["input_ids"] = teacher_input_ids

                                    # Never unmask more tokens than the original student response.
                                    teacher_attention_mask = base_batch.batch["attention_mask"].clone()
                                    selected_valid_mask = (selected_teacher_responses != teacher_pad_id).to(
                                        teacher_attention_mask.dtype
                                    )
                                    selected_valid_mask = selected_valid_mask * teacher_attention_mask[:, -response_len:]
                                    teacher_attention_mask[:, -response_len:] = selected_valid_mask
                                    out.batch["attention_mask"] = teacher_attention_mask
                                    if "response_mask" in out.batch.keys():
                                        out.batch["response_mask"] = selected_valid_mask
                                    return out

                                def _compute_overlap_ratio(teacher_output: DataProto, response_mask: torch.Tensor):
                                    if "overlap_mask" not in teacher_output.batch.keys():
                                        return None
                                    overlap_mask = teacher_output.batch["overlap_mask"].float()
                                    valid_mask = response_mask.unsqueeze(-1).expand_as(overlap_mask).float()
                                    denom = valid_mask.sum()
                                    if denom.item() <= 0:
                                        return 0.0
                                    return float(((overlap_mask * valid_mask).sum() / (denom + 1e-6)).item())

                                rm_score_batch_selected = None
                                if selected_teacher_responses is not None and (
                                    apply_selected_for_teacher_forward or multi_forward_select_by_overlap
                                ):
                                    rm_score_batch_selected = _build_selected_forward_batch(batch)

                                if multi_forward_select_by_overlap and rm_score_batch_selected is not None and top_k > 0:
                                    teacher_data_student = self.rm_wg.compute_rm_score(batch)
                                    teacher_data_selected = self.rm_wg.compute_rm_score(rm_score_batch_selected)

                                    overlap_student = _compute_overlap_ratio(
                                        teacher_data_student, batch.batch["response_mask"]
                                    )
                                    overlap_selected = _compute_overlap_ratio(
                                        teacher_data_selected, rm_score_batch_selected.batch["response_mask"]
                                    )
                                    if overlap_student is not None:
                                        metrics["teacher_selection/forward_overlap_student_ctx"] = overlap_student
                                    if overlap_selected is not None:
                                        metrics["teacher_selection/forward_overlap_selected_ctx"] = overlap_selected

                                    if overlap_student is None or overlap_selected is None:
                                        # No overlap tensors (e.g. top_k disabled in worker path):
                                        # keep legacy preference.
                                        use_selected_trial = bool(apply_selected_for_teacher_forward)
                                    else:
                                        use_selected_trial = bool(overlap_selected > overlap_student)
                                    metrics["teacher_selection/forward_trial_selected_ctx"] = (
                                        1.0 if use_selected_trial else 0.0
                                    )

                                    teacher_data = teacher_data_selected if use_selected_trial else teacher_data_student
                                else:
                                    rm_score_batch = batch
                                    if rm_score_batch_selected is not None and apply_selected_for_teacher_forward:
                                        rm_score_batch = rm_score_batch_selected
                                    teacher_data = self.rm_wg.compute_rm_score(rm_score_batch)
                                batch = batch.union(teacher_data)

                                # Optional auxiliary KD term on selected teacher context:
                                #   L_total = L_student_ctx + lambda * L_teacher_ctx
                                # where L_teacher_ctx uses student/teacher forward both on
                                # selected teacher rollout context.
                                aux_teacher_ctx_kd_enable = bool(
                                    self.config.actor_rollout_ref.rollout.get("aux_teacher_ctx_kd_enable", False)
                                )
                                aux_teacher_ctx_kd_coef = float(
                                    self.config.actor_rollout_ref.rollout.get("aux_teacher_ctx_kd_coef", 0.0)
                                )
                                aux_teacher_ctx_kd_weight_mode = self.config.actor_rollout_ref.rollout.get(
                                    "aux_teacher_ctx_kd_weight_mode", reward_weight_mode
                                )
                                batch.meta_info["aux_teacher_ctx_kd_enable"] = (
                                    aux_teacher_ctx_kd_enable and aux_teacher_ctx_kd_coef > 0
                                )
                                batch.meta_info["aux_teacher_ctx_kd_coef"] = aux_teacher_ctx_kd_coef
                                batch.meta_info["aux_teacher_ctx_kd_weight_mode"] = aux_teacher_ctx_kd_weight_mode

                                aux_teacher_ctx_ready = False
                                if aux_teacher_ctx_kd_enable and aux_teacher_ctx_kd_coef > 0:
                                    if strategy != "only_stu":
                                        # Keep the auxiliary path simple and robust: it is defined
                                        # for student-candidate top-k supervision.
                                        print(
                                            f"[AuxTeacherCtxKD] skip at step {self.global_steps}: "
                                            f"top_k_strategy={strategy} (supported: only_stu)"
                                        )
                                    elif top_k <= 0:
                                        print(
                                            f"[AuxTeacherCtxKD] skip at step {self.global_steps}: "
                                    ...
                                    elif selected_teacher_responses is None:
                                        print(
                                            f"[AuxTeacherCtxKD] skip at step {self.global_steps}: "
                                            "selected_teacher_responses is None"
                                        )
                                    else:
                                        # Build a clean selected-context batch (without old_log_probs / top-k
                                        # tensors from student context), then compute:
                                        #   1) student top-k ids/log-probs on selected context
                                        #   2) teacher log-probs on those selected-context student ids
                                        aux_selected_batch = _build_selected_forward_batch(
                                            batch, include_all_batch_keys=False
                                        )
                                        aux_student_log_prob = self.actor_rollout_wg.compute_log_prob(aux_selected_batch)
                                        aux_selected_batch = aux_selected_batch.union(aux_student_log_prob)
                                        aux_teacher_data = self.rm_wg.compute_rm_score(aux_selected_batch)

                                        if "teacher_on_student_log_probs" in aux_teacher_data.batch.keys():
                                            aux_tensors = {
                                                "aux_teacher_ctx_input_ids": aux_selected_batch.batch["input_ids"],
                                                "aux_teacher_ctx_attention_mask": aux_selected_batch.batch["attention_mask"],
                                                "aux_teacher_ctx_position_ids": aux_selected_batch.batch["position_ids"],
                                                "aux_teacher_ctx_responses": aux_selected_batch.batch["responses"],
                                                "aux_teacher_ctx_response_mask": aux_selected_batch.batch["response_mask"],
                                                "aux_teacher_ctx_student_top_k_ids": aux_selected_batch.batch["student_top_k_ids"],
                                                "aux_teacher_ctx_teacher_on_student_log_probs": aux_teacher_data.batch[
                                                    "teacher_on_student_log_probs"
                                                ],
                                            }
                                            # Pass teacher's own top-K so dp_actor can use
                                            # KL(p_teacher || p_student) on teacher-confident tokens.
                                            if "teacher_top_k_ids" in aux_teacher_data.batch.keys():
                                                aux_tensors["aux_teacher_ctx_teacher_top_k_ids"] = aux_teacher_data.batch["teacher_top_k_ids"]
                                            if "teacher_top_k_log_probs" in aux_teacher_data.batch.keys():
                                                aux_tensors["aux_teacher_ctx_teacher_top_k_log_probs"] = aux_teacher_data.batch["teacher_top_k_log_probs"]
                                            aux_payload = DataProto.from_dict(tensors=aux_tensors)
                                            batch = batch.union(aux_payload)
                                            aux_teacher_ctx_ready = True
                                        else:
                                            print(
                                                f"[AuxTeacherCtxKD] skip at step {self.global_steps}: "
                                                "teacher_on_student_log_probs missing in aux teacher data"
                                            )

                                metrics["teacher_selection/aux_teacher_ctx_kd_enabled"] = (
                                    1.0 if (aux_teacher_ctx_kd_enable and aux_teacher_ctx_kd_coef > 0) else 0.0
                                )
                                metrics["teacher_selection/aux_teacher_ctx_kd_ready"] = (
                                    1.0 if aux_teacher_ctx_ready else 0.0
                                )
                                metrics["teacher_selection/aux_teacher_ctx_kd_coef"] = float(aux_teacher_ctx_kd_coef)
                            ...
                            if top_k > 0:
                                ...
                                # Teacher-side top-K/log-probs are produced from the chosen
                                # teacher forward trial via compute_rm_score above.
