...
def _resolve_attn_implementation(
    attn_implementation: str, model_dtype: Optional[Any] = None, rank: Optional[int] = None
) -> str:
    """Fallback to sdpa when flash_attention_2 is unavailable or incompatible."""
    if attn_implementation != "flash_attention_2":
        return attn_implementation

    normalized_dtype = model_dtype
    if isinstance(model_dtype, str):
        dtype_name = model_dtype.lower()
    ...
    if normalized_dtype not in (None, torch.float16, torch.bfloat16):
        if rank in (None, 0):
            logger.warning(
                "flash_attention_2 requested but model dtype is %s; falling back to sdpa.",
                normalized_dtype,
            )
        return "sdpa"
    ...
    try:
        ...
        flash2_available = is_flash_attn_2_available()
    except Exception:
        flash2_available = False

    if flash2_available:
        # `is_flash_attn_2_available()` can still return True when the Python package
        # exists but its CUDA extension is ABI-incompatible with the current torch/cuda.
        try:
            from flash_attn import flash_attn_func, flash_attn_varlen_func  # noqa: F401
        except Exception as exc:
            if rank in (None, 0):
                logger.warning(
                    "flash_attention_2 appears installed but failed to import CUDA kernels (%s); "
...
# --- Teacher rollout hinted-retry helpers ---------------------------------
# Used by RewardModelWorker.generate_teacher_rollouts_with_hint to embed the
# ground-truth answer into the prompt as a "silent validation key" so that
# the teacher can reach the right answer without leaking it in the response.
_SILENT_KEY_HINT = (
    "[SILENT_VALIDATION_KEY — DO NOT MENTION IN THINKING OR RESPONSE: {gt}]\n\n"
    "STRICT RULES about the validation key:\n"
    "  1. NEVER mention, quote, paraphrase, or allude to it anywhere — not in <think>, not in your answer.\n"
    "  2. NEVER say things like 'the key says', 'based on the hint', 'the answer is given', "
    "'I can see the correct answer is', or any equivalent phrasing.\n"
    "  3. Your entire chain of thought must be derived from what you observe in the problem.\n"
    "  4. Only use the validation key silently as a final sanity-check after you have already "
    "reasoned to a conclusion — never as a starting point or shortcut."
)

# Markers we try (in order) to locate the assistant generation turn and
# inject the hint as an extra user/system note immediately before it.
# Covers Qwen2 (<|im_start|>assistant), DeepSeek-R1 distill variants
# (<｜Assistant｜>, both full-width and ASCII), and a generic "Assistant:"
# fallback.
_ASSISTANT_MARKERS = (
    "<｜Assistant｜>",
    "<|Assistant|>",
...
def _build_silent_key_hint(gt: str) -> str:
    return _SILENT_KEY_HINT.format(gt=gt)


def _inject_hint_before_assistant(prompt_text: str, hint_text: str) -> str:
    for marker in _ASSISTANT_MARKERS:
        if marker in prompt_text:
            pre, _, post = prompt_text.rpartition(marker)
            return pre.rstrip() + "\n\n" + hint_text + "\n\n" + marker + post
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        attn_implementation = _resolve_attn_implementation(
            attn_implementation, model_dtype=torch_dtype, rank=self.rank
        )
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if has_remote_code:
                ...
                if model_type in AutoModelForVision2Seq._model_mapping._model_mapping:
                    actor_module_class = AutoModelForVision2Seq
                elif model_type in AutoModelForCausalLM._model_mapping._model_mapping:
                    actor_module_class = AutoModelForCausalLM
                elif model_type in AutoModelForImageTextToText._model_mapping._model_mapping:
                    actor_module_class = AutoModelForImageTextToText
                else:
                    actor_module_class = AutoModel
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        attn_implementation = override_config.get("attn_implementation", "flash_attention_2")
        attn_implementation = _resolve_attn_implementation(
            attn_implementation, model_dtype=model_dtype, rank=self.rank
        )
    ...
    def _compute_entropy_safe(self, logits, chunk_size=4096):
        ...
        # Flatten to [-1, vocab_size]; use reshape because upstream slices/permutations
    ...
    def init_model(self):
        ...
        # Build a non-FSDP generation model for teacher rollouts when enabled.
        # A separate copy is required because FSDP with CPU offload is far too
        # slow for autoregressive generation (one all-gather per token step).
        teacher_rollout_cfg = self.config.get("teacher_rollout", {})
        if teacher_rollout_cfg.get("enable", False):
            self._build_gen_model()

    def _build_gen_model(self):
        """Load a non-FSDP copy of the teacher model for autoregressive generation.

        Each DP rank gets a full model copy on its own GPU.  For a 1.5 B model
        this is ~3 GB bfloat16, which is well within A100 capacity alongside the
        sharded FSDP reward module.
        """
        import torch.nn.functional as F  # noqa: F401 – ensure F is available in method scope
        from transformers import AutoConfig, AutoModelForCausalLM
        ...
        self.gen_tokenizer = hf_tokenizer(
        ...
        self.gen_model.eval()
        if self.rank == 0:
            print(f"[TeacherRollout] Loaded non-FSDP generation model from {local_path}")

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="reward"))
    def generate_teacher_rollouts(self, data: DataProto) -> DataProto:
        """Generate M teacher rollout responses per prompt.

        Each DP rank generates for its local batch shard.  Results are gathered
        by the driver via the standard dispatch/collect mechanism.

        meta_info keys consumed:
            teacher_n_rollouts (int, default 4)
            teacher_max_new_tokens (int, default 2048)
            teacher_gen_temperature (float, default 0.7)
            teacher_top_p (float, default 0.95)

        Returns:
            DataProto with ``teacher_response_ids`` of shape
            (local_BS * M, max_new_tokens).
        """
        import torch.nn.functional as F

        assert hasattr(self, "gen_model"), (
            "gen_model not initialised; set reward_model.teacher_rollout.enable=True"
        )

        n_rollouts = data.meta_info.get("teacher_n_rollouts", 4)
        max_new_tokens = data.meta_info.get("teacher_max_new_tokens", 2048)
        temperature = data.meta_info.get("teacher_gen_temperature", 0.7)
        top_p = data.meta_info.get("teacher_top_p", 0.95)
        disturb_enable = data.meta_info.get("teacher_prompt_disturb_enable", False)
        disturb_rollout_index = int(data.meta_info.get("teacher_prompt_disturb_rollout_index", 2))
        disturb_text = data.meta_info.get("teacher_prompt_disturb_text", "")

        data = data.to(get_device_id())
        input_ids = data.batch["input_ids"]    # (BS, full_len)
        responses = data.batch["responses"]    # (BS, resp_len)

        resp_len = responses.shape[1]
        prompt_len = input_ids.shape[1] - resp_len

        prompt_ids = input_ids[:, :prompt_len]                      # (BS, prompt_len)
        prompt_attn = data.batch["attention_mask"][:, :prompt_len]  # (BS, prompt_len)

        if disturb_enable and disturb_text and 1 <= disturb_rollout_index <= n_rollouts:
            # Build (BS * n_rollouts) prompts; rollout slot disturb_rollout_index-1 (0-indexed)
            # gets the disturb text appended, all other slots keep the original prompt.
            disturb_ids = self.gen_tokenizer.encode(disturb_text, add_special_tokens=False)
            device = prompt_ids.device
            pad_id = self.gen_tokenizer.pad_token_id
            disturb_idx_0 = disturb_rollout_index - 1  # 0-indexed slot

            all_input_ids: list[list[int]] = []
            all_attn_masks: list[list[int]] = []
            for i in range(prompt_ids.shape[0]):
                for slot in range(n_rollouts):
                    if slot == disturb_idx_0:
                        actual_ids = prompt_ids[i][prompt_attn[i].bool()].tolist()
                        new_ids = actual_ids + disturb_ids
                        all_input_ids.append(new_ids)
                        all_attn_masks.append([1] * len(new_ids))
                    else:
                        all_input_ids.append(prompt_ids[i].tolist())
                        all_attn_masks.append(prompt_attn[i].tolist())

            max_len = max(len(ids) for ids in all_input_ids)
            prompt_ids_rep = torch.tensor(
                [[pad_id] * (max_len - len(ids)) + ids for ids in all_input_ids],
                dtype=torch.long, device=device,
            )
            prompt_attn_rep = torch.tensor(
                [[0] * (max_len - len(a)) + a for a in all_attn_masks],
                dtype=torch.long, device=device,
            )
            logger.info(
                "teacher rollout prompt disturb enabled: rollout_index=%d text=%r",
                disturb_rollout_index, disturb_text,
            )
        else:
            # Standard path: no perturbation — simple repeat_interleave.
            prompt_ids_rep = prompt_ids.repeat_interleave(n_rollouts, dim=0)
            prompt_attn_rep = prompt_attn.repeat_interleave(n_rollouts, dim=0)

        with torch.no_grad():
            outputs = self.gen_model.generate(
                input_ids=prompt_ids_rep,
                attention_mask=prompt_attn_rep,
        ...
        gen_ids = outputs[:, prompt_ids_rep.shape[1]:]  # (BS*M, actual_len)

        # Pad or truncate to max_new_tokens
        actual_len = gen_ids.shape[1]
        if actual_len < max_new_tokens:
            gen_ids = F.pad(gen_ids, (0, max_new_tokens - actual_len), value=self.gen_tokenizer.pad_token_id)
        else:
            gen_ids = gen_ids[:, :max_new_tokens]

        return DataProto.from_dict(tensors={"teacher_response_ids": gen_ids.cpu()})

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="reward"))
    def generate_teacher_rollouts_with_hint(self, data: DataProto) -> DataProto:
        """Generate M' teacher rollouts with a SILENT_VALIDATION_KEY hint injected.

        The ground-truth answer is placed into the prompt as a "silent validation
        key" that the model is instructed not to reveal. Used as tier-2 retry
        for samples where unhinted rollouts produced no correct answer.

        meta_info keys consumed:
            teacher_n_rollouts_hint (int, default 2)
            teacher_max_new_tokens  (int, default 2048)
            teacher_gen_temperature (float, default 0.7)
            teacher_top_p           (float, default 0.95)

        non_tensor_batch["reward_model"][i]["ground_truth"] provides the key.

        Returns:
            DataProto with ``teacher_response_ids`` (BS*M', max_new_tokens).
        """
        import torch.nn.functional as F
        ...
        n_rollouts = data.meta_info.get("teacher_n_rollouts_hint", 2)
        max_new_tokens = data.meta_info.get("teacher_max_new_tokens", 2048)
        temperature = data.meta_info.get("teacher_gen_temperature", 0.7)
        top_p = data.meta_info.get("teacher_top_p", 0.95)
        ...
        rm_info = data.non_tensor_batch.get("reward_model", None)

        # Decode each prompt, inject the hint before the assistant-turn marker,
        # re-tokenize. Collect as a variable-length list, then left-pad to batch.
        hinted_list = []
        max_hinted_len = 0
        for i in range(BS):
            gt = ""
            ...
            hint_txt = _build_silent_key_hint(gt)
            # Decode only valid prompt tokens (exclude left-padding). Keeping
            # special tokens is necessary to preserve assistant-turn markers.
            valid_prompt_ids = input_ids[i, :prompt_len][prompt_attn[i].bool()]
            if valid_prompt_ids.numel() == 0:
                valid_prompt_ids = input_ids[i, :prompt_len]
            pi_text = self.gen_tokenizer.decode(
                valid_prompt_ids.tolist(), skip_special_tokens=False
            )
            hinted_text = _inject_hint_before_assistant(pi_text, hint_txt)
            hinted_ids = self.gen_tokenizer(
                hinted_text, return_tensors="pt", add_special_tokens=False
            ).input_ids[0].to(device)
            hinted_list.append(hinted_ids)
            if hinted_ids.shape[0] > max_hinted_len:
                max_hinted_len = int(hinted_ids.shape[0])

        hinted_padded = torch.full((BS, max_hinted_len), pad_id, dtype=torch.long, device=device)
        hinted_attn = torch.zeros((BS, max_hinted_len), dtype=torch.long, device=device)
        for i, hids in enumerate(hinted_list):
            Li = hids.shape[0]
            hinted_padded[i, -Li:] = hids
            hinted_attn[i, -Li:] = 1

        prompt_ids_rep = hinted_padded.repeat_interleave(n_rollouts, dim=0)
        prompt_attn_rep = hinted_attn.repeat_interleave(n_rollouts, dim=0)
        ...
        gen_ids = outputs[:, prompt_ids_rep.shape[1]:]
        actual_len = gen_ids.shape[1]
        if actual_len < max_new_tokens:
            gen_ids = F.pad(gen_ids, (0, max_new_tokens - actual_len), value=pad_id)
        else:
            gen_ids = gen_ids[:, :max_new_tokens]
