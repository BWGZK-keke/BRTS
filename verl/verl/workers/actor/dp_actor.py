...
class DataParallelPPOActor(BasePPOActor):
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    # Optimization: when top_k > 0, avoid materializing full-vocab
                    # log_softmax(logits) because it is very memory heavy for long
                    # sequences. Use:
                    #   log p(token) = logit(token) - logsumexp(all logits)
                    # which is mathematically equivalent and much lighter.
                    need_topk = top_k > 0
                    if need_topk:
                        log_norm = torch.logsumexp(logits_rmpad, dim=-1, keepdim=True)
                        target_logits = logits_rmpad.gather(dim=-1, index=input_ids_rmpad_rolled.unsqueeze(-1))
                        log_probs = (target_logits - log_norm).squeeze(-1)
                    else:
    ...
    def update_policy(self, data: DataProto):
        ...
        temperature = data.meta_info["temperature"]  # temperature must be in the data.meta_info to avoid silent error
        aux_teacher_ctx_kd_enable = bool(data.meta_info.get("aux_teacher_ctx_kd_enable", False))
        aux_teacher_ctx_kd_coef = float(data.meta_info.get("aux_teacher_ctx_kd_coef", 0.0))
        aux_teacher_ctx_kd_weight_mode = data.meta_info.get("aux_teacher_ctx_kd_weight_mode", "student_p")
        ...
        aux_teacher_ctx_keys = [
            "aux_teacher_ctx_input_ids",
            "aux_teacher_ctx_attention_mask",
            "aux_teacher_ctx_position_ids",
            "aux_teacher_ctx_responses",
            "aux_teacher_ctx_response_mask",
            "aux_teacher_ctx_student_top_k_ids",
            "aux_teacher_ctx_teacher_on_student_log_probs",
        ]
        # Optional keys present when teacher_topk weight mode is used.
        aux_teacher_topk_keys = [
            "aux_teacher_ctx_teacher_top_k_ids",
            "aux_teacher_ctx_teacher_top_k_log_probs",
        ]
        has_aux_teacher_ctx_payload = all(k in data.batch.keys() for k in aux_teacher_ctx_keys)
        if has_aux_teacher_ctx_payload:
            select_keys.extend(aux_teacher_ctx_keys)
            for k in aux_teacher_topk_keys:
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # Backpropagate main policy loss first.
                    # This frees all main-forward activations and fully unwinds the
                    # activation-offload handler (current_group back to 0), leaving it
                    # clean for a second forward pass.
                    (policy_loss * loss_scale_factor).backward()
                    micro_batch_metrics["actor/pg_loss"] = pg_loss.detach().item() * loss_scale_factor

                    # Auxiliary selected-context KD: L_aux = lambda * L_teacher_ctx
                    # Run as a separate forward+backward after the main backward so that
                    # activation offload and gradient checkpointing work normally in
                    # training mode (no eval() workaround needed).  Gradients accumulate
                    # into .grad alongside the main-loss gradients — mathematically
                    # identical to a single backward of (L_main + lambda * L_aux).
                    if aux_teacher_ctx_kd_enable and aux_teacher_ctx_kd_coef > 0 and has_aux_teacher_ctx_payload:
                        aux_input_ids = model_inputs["aux_teacher_ctx_input_ids"]
                        aux_attention_mask = model_inputs["aux_teacher_ctx_attention_mask"]
                        aux_position_ids = model_inputs["aux_teacher_ctx_position_ids"]
                        aux_responses = model_inputs["aux_teacher_ctx_responses"]
                        aux_response_mask = model_inputs["aux_teacher_ctx_response_mask"]

                        aux_model_inputs = {
                            "input_ids": aux_input_ids,
                            "attention_mask": aux_attention_mask,
                            "position_ids": aux_position_ids,
                            "responses": aux_responses,
                        }
                        if "multi_modal_inputs" in model_inputs:
                            aux_model_inputs["multi_modal_inputs"] = model_inputs["multi_modal_inputs"]

                        # teacher_topk mode: KL(p_teacher || p_student) on teacher-confident tokens.
                        # teacher's own top-K ids/log-probs are passed from ray_trainer when available.
                        use_teacher_topk = (
                            aux_teacher_ctx_kd_weight_mode == "teacher_topk"
                            and "aux_teacher_ctx_teacher_top_k_ids" in model_inputs
                            and "aux_teacher_ctx_teacher_top_k_log_probs" in model_inputs
                        )

                        if use_teacher_topk:
                            # Target tokens = teacher's top-K; reference = teacher's log-probs
                            aux_target_ids = model_inputs["aux_teacher_ctx_teacher_top_k_ids"]
                            aux_teacher_logp = model_inputs["aux_teacher_ctx_teacher_top_k_log_probs"]
                        else:
                            # Fallback: student's top-K ids; teacher's log-probs at those positions
                            aux_target_ids = model_inputs["aux_teacher_ctx_student_top_k_ids"]
                            aux_teacher_logp = model_inputs["aux_teacher_ctx_teacher_on_student_log_probs"]

                        _, _, _, aux_student_logp = self._forward_micro_batch(
                            aux_model_inputs,
                            temperature=temperature,
                            calculate_entropy=False,
                            top_k=aux_target_ids.shape[-1],
                            student_top_k_ids=aux_target_ids,
                        )

                        aux_valid_mask = aux_response_mask.unsqueeze(-1).expand_as(aux_student_logp).bool()
                        if use_teacher_topk or aux_teacher_ctx_kd_weight_mode == "teacher_topk":
                            # KL(p_teacher || p_student): weight by teacher's distribution over its top-K
                            aux_weight_logits = aux_teacher_logp
                        elif aux_teacher_ctx_kd_weight_mode == "student_p":
                            aux_weight_logits = aux_student_logp.detach()
                        elif aux_teacher_ctx_kd_weight_mode == "teacher_p":
                            aux_weight_logits = aux_teacher_logp
                        elif aux_teacher_ctx_kd_weight_mode == "none":
                            aux_weight_logits = torch.zeros_like(aux_student_logp)
                        else:
                            raise ValueError(
                                f"Unknown aux_teacher_ctx_kd_weight_mode: {aux_teacher_ctx_kd_weight_mode}"
                            )

                        aux_weight_logits = torch.where(
                            aux_valid_mask, aux_weight_logits, torch.full_like(aux_weight_logits, -float("inf"))
                        )
                        aux_log_weights = aux_weight_logits - torch.logsumexp(aux_weight_logits, dim=-1, keepdim=True)
                        aux_weights = torch.exp(aux_log_weights)
                        aux_weights = torch.nan_to_num(aux_weights, nan=0.0, posinf=0.0, neginf=0.0)

                        if use_teacher_topk:
                            # KL(p_t||p_s) = Σ p_t * (log_pt - log_ps)
                            aux_kl = aux_teacher_logp - aux_student_logp
                        else:
                            # Forward KL on student top-K: Σ w * (log_ps - log_pt) → aligns student to teacher
                            aux_kl = aux_student_logp - aux_teacher_logp
                        aux_kl = torch.where(aux_valid_mask, aux_kl, torch.zeros_like(aux_kl))
                        aux_kl_token = (aux_weights * aux_kl).sum(dim=-1)
                        aux_kd_loss = agg_loss(
                            loss_mat=aux_kl_token, loss_mask=aux_response_mask, loss_agg_mode=loss_agg_mode
                        )

                        (aux_teacher_ctx_kd_coef * aux_kd_loss * loss_scale_factor).backward()

                        micro_batch_metrics["actor/aux_teacher_ctx_kd_loss"] = (
                            aux_kd_loss.detach().item() * loss_scale_factor
                        )
                        micro_batch_metrics["actor/aux_teacher_ctx_kd_coef"] = aux_teacher_ctx_kd_coef
