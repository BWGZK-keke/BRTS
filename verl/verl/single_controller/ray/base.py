...
class RayClassWithInitArgs(ClassWithInitArgs):
    ...
    def __call__(
        ...
        sharing_with=None,
        device_name="cuda",
        worker_env_vars: Optional[dict[str, str]] = None,
    ) -> Any:
        ...
        if sharing_with is not None:
            ...
            remote_kwargs["cuda_visible_devices"] = visible_devices
            if worker_env_vars is not None:
                remote_kwargs["worker_env_vars"] = worker_env_vars
...
class RayWorkerGroup(WorkerGroup):
    ...
    def _init_with_resource_pool(self, resource_pool, ray_cls_with_init, bin_pack, detached, worker_env=None):
        ...
        disable_runtime_env = os.environ.get("VERL_DISABLE_RAY_RUNTIME_ENV", "0") == "1"
        ...
        for pg_idx, pg in enumerate(sort_placement_group_by_node_ip(pgs)):
            ...
            for local_rank in range(local_world_size):
                ...
                if disable_runtime_env:
                    ray_cls_with_init.update_options({"name": name})
                elif self.profile_steps and self.device_name == "cuda":
                ...
                worker = ray_cls_with_init(
                    ...
                    worker_env_vars=env_vars if disable_runtime_env else None,
                )
...
def create_colocated_worker_cls(class_dict: dict[str, RayClassWithInitArgs]):
    ...
    class WorkerDict(worker_cls):
        def __init__(self, *args, worker_env_vars=None, **kwargs):
            # Compatibility path for environments where Ray runtime_env is disabled:
            # inject worker env vars directly into process env before worker init.
            if worker_env_vars:
                for key, value in worker_env_vars.items():
                    os.environ[key] = str(value)
            try:
                super().__init__(*args, worker_env_vars=worker_env_vars, **kwargs)
            except TypeError as e:
                # Backward compatibility for older Worker base signatures.
                if "worker_env_vars" in str(e):
